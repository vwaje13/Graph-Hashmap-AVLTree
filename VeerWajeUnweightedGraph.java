...
public class VeerWajeUnweightedGraph<V> implements VeerWajeGraph<V> {
  protected List<V> vertices = new ArrayList<>(); // Store vertices
  protected List<List<Edge>> neighbors = new ArrayList<>(); // Adjacency Edge lists

  /** Construct an empty graph */
  protected VeerWajeUnweightedGraph() {
  }

  /** Construct a graph from vertices and edges stored in arrays */
  protected VeerWajeUnweightedGraph(V[] vertices, int[][] edges) {
  ...
  protected VeerWajeUnweightedGraph(List<V> vertices, List<Edge> edges) {
  ...
  protected VeerWajeUnweightedGraph(List<Edge> edges, int numberOfVertices) {
  ...
  protected VeerWajeUnweightedGraph(int[][] edges, int numberOfVertices) {
    for (int i = 0; i < numberOfVertices; i++) 
      addVertex((V)(Integer.valueOf(i))); // vertices is {0, 1, ... }
...
//CLASS END
  ...
  public boolean remove(V v) {
      ...
      if (indexToRemove != -1) {
          List<Edge> edgesToRemove = neighbors.get(indexToRemove);

          // Remove all edges associated with the vertex v
          for (Edge edge : edgesToRemove) {
              int neighborIndex = (int) edge.v();
              neighbors.get(neighborIndex).removeIf(e -> e.v() == indexToRemove);
          }

          // Remove the vertex from the list of vertices and its associated edges
  ...
  public boolean remove(int u, int v) {
      ...
      if (indexU != -1 && indexV != -1) {
          List<Edge> edgesU = neighbors.get(indexU);
          List<Edge> edgesV = neighbors.get(indexV);

          Edge edgeToRemove = null;

          // Find the edge to remove
          for (Edge edge : edgesU) {
          ...
          // Remove the edge from both lists
          if (edgeToRemove != null) {
              edgesU.remove(edgeToRemove);
